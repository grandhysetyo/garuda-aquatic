            ...
            <div class="block w-full overflow-x-auto">
                ...
                <table class="items-center w-full bg-transparent border-collapse">
                    ...
                    <tbody>
                        ...
                        @foreach ($gallerys as $gallery)                        
                            <tr>
                              <th
                                  ...
                                  <img src="/images/gallery/{{ $gallery->filename }}"
                              ...
                              <td
                                  class="border-t-0 px-6 align-middle border-l-0 border-r-0 text-xs whitespace-nowrap p-4">                                
                                  @if ($gallery->category == 0)
                                    Our Farm
                                  @elseif ($gallery->category == 1)
                                    Product                                  
                                  @endif
                              </td>
                              <td
                                  ...
                                  <div class="flex">
                                      ...
                                      <button class="text-red-600 block py-1 px-3" type="button" onclick="toggleModal('delete','{{ route('delete-gallery', $gallery->id) }}')">
                                          <i class="fas fa-trash"></i>
                                      </button>
                                      @if ($gallery->status == 1)
                                          <button class="text-blue-600 block py-1 px-3" type="button" onclick="toggleModal('deactive','{{ route('toggle-gallery', ['id' => $gallery->id, 'status' => 0]) }}')">
                                              <i class="fas fa-eye"></i>
                                          </button>
                                      @else
                                          <button class="text-blue-600 block py-1 px-3" type="button" onclick="toggleModal('active','{{ route('toggle-gallery', ['id' => $gallery->id, 'status' => 1]) }}')">
                      ...
                      @else
                        <tr>
                          <td colspan="5" class="text-center py-9">No data found</td>
                        </tr>
                      @endif        
                    </tbody>
                </table>
...
@section('script')
  <script type="text/javascript">
    ...
    function toggleModal(type, url){
        ...
        console.log(url)
        if(type === 'delete'){
            $('#modal-title').html("Delete gallery");
            $('#modal-body').html("Are u sure want to delete this gallery?");
            $('#modal-url').html("Delete");
        }
        else if(type === 'active'){
            $('#modal-title').html("Active gallery");
            $('#modal-body').html("Are u sure want to active this gallery?");
            $('#modal-url').html("Active");
        } 
        else if(type === 'deactive'){
            $('#modal-title').html("Deactive gallery");
            $('#modal-body').html("Are u sure want to deactive this gallery?");
